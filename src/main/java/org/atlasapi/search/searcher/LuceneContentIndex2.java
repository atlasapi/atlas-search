...
public class LuceneContentIndex2 implements ContentChangeListener, DebuggableContentSearcher {

    private static final int MAX_RESULTS = 1000;
    private static final Log log = LogFactory.getLog(LuceneContentIndex2.class);
    ...
    private final Directory writebackDir;
    private final ExecutorService writebackExceutor;
    ...
    public LuceneContentIndex2(File luceneDir, KnownTypeContentResolver contentResolver) {
        this.contentResolver = contentResolver;
        try {
            this.refreshLock = new ReentrantReadWriteLock();
            //
            this.writebackExceutor = new ThreadPoolExecutor(1, 1, 1, TimeUnit.DAYS, new LinkedBlockingQueue(100), new ThreadPoolExecutor.CallerRunsPolicy());
            this.writebackDir = FSDirectory.open(luceneDir);
            optimizeIndex(writebackDir);
            //
            this.contentDir = new RAMDirectory(writebackDir);
    ...
    public void contentChange(Iterable<? extends Described> contents) {
        // This is an implementation detail: we're doing a preventive copy, because contents come from a remote store,
        // and we want to have them materilized and copied upfront to avoid later IO errors.
        final Iterable<? extends Described> copyOfContents = Lists.newLinkedList(contents);
        //
        index(contentDir, copyOfContents);
        writebackExceutor.submit(new Runnable() {

            @Override
            public void run() {
                index(writebackDir, copyOfContents);
            }
        });
    }
    ...
    public void afterContentChange() {
        optimizeIndex(contentDir);
        refreshSearcher(contentDir);
    }

    private void index(Directory directory, Iterable<? extends Described> contents) throws RuntimeException {
        IndexWriter writer = null;
        try {
            writer = writerFor(directory);
    ...
    private void optimizeIndex(Directory directory) throws RuntimeException {
        IndexWriter writer = null;
        try {
            writer = writerFor(directory);
            // commit called here to avoid a bug with empty indexes
            writer.commit();
            //
            writer.optimize();
        } catch (Exception e) {
    ...
    private IndexWriter writerFor(Directory dir) throws CorruptIndexException, LockObtainFailedException, IOException {
    ...
    private void refreshSearcher(Directory directory) {
        ...
        try {
            ...
            try {
                this.contentSearcher = new IndexSearcher(directory);
